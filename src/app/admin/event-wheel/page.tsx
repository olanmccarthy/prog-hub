...
export default function EventWheelPage() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const result = editingId
        ? await updateEventWheelEntry(editingId, formData)
        : await createEventWheelEntry(formData);

      if (result.success) {
        setSuccess(editingId ? 'Entry updated successfully' : 'Entry created successfully');
        handleCloseDialog();
        await loadConfigEntries();
        await loadStatus(); // Refresh wheel display
      } else {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      setError(null);
      setSuccess(null);
      const result = await deleteEventWheelEntry(id);
  ...
  const handleSaveMassEdit = async () => {
    try {
      ...
      const result = await massUpdateEventWheelChances({ updates });
  ...
  const handleApplyMultiplier = async () => {
    ...
    try {
      ...
      const result = await applyMultiplierToEventWheelEntries({
